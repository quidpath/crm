...
class JWTAuthenticationMiddleware:
    ...
    def __call__(self, request):
        ...
        service_secret = os.environ.get("CRM_SERVICE_SECRET") or getattr(settings, "CRM_SERVICE_SECRET", "")
        if service_secret and self._is_service_to_service_path(request.path):
            if request.META.get("HTTP_X_SERVICE_KEY", "").strip() == service_secret:
        ...
        try:
            ...
            request.user_data = {
                ...
                "email": payload["email"], "role": payload.get("role"),
            ...
            try:
                ...
                request.corporate_data = (
                    self.cache_service.get_corporate_data(request.corporate_id) if request.corporate_id else None
                )
    ...
    def _is_public_endpoint(self, path):
        return any(path.startswith(p) for p in ["/health/", "/admin/", "/static/", "/media/"])
